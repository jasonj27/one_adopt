...
namespace :access do
  ...
  task :shelter_api, [:quantity] => [:environment] do |task, args|
    ...
    # 找出已建立的收容所user
    shelter = User.where("email ILIKE :email", email: "%pet.com").map { |x| { x.name => [x.id, x.latitude, x.longitude] } }.inject(:merge)
    puts shelter
    ...
    @result.sample(quantity).each do |result|
      ...
      if Animal.find_by(animal_subid: result["animal_subid"])
        ...
        unless animal_data.user_id == shelter[result["shelter_name"]][0].to_i
          animal_data.assign_attributes(user_id: shelter[result["shelter_name"]].to_i)
        end
        ...
        animal_data.assign_attributes(
          ...
          user_id: shelter[result["shelter_name"]][0].to_i,
          adopt_status: result["adopt_status"],
          animal_bodytype: result["animal_bodytype"],
          animal_colour: result["animal_colour"],
          gov_shelter: true,
          latitude: shelter[result["shelter_name"]][1].to_f,
          longitude: shelter[result["shelter_name"]][2].to_f,
        )
      end
      ...
      puts "#{animal_data.latitude}"
      puts "#{animal_data.longitude}"
    end
    puts "Finish!!!"
  end
end
